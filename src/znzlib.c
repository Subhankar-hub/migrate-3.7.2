...
{
  znzFile file;
  file = (znzFile) mycalloc(1L,sizeof(struct znzptr));
  if( file == NULL ){
...
int znzbuffer(znzFile file, unsigned long size)
{
#ifdef  HAVE_GZBUFFER 
...
{
  znzFile file;
  file = (znzFile) mycalloc(1,sizeof(struct znzptr));
  if( file == NULL ){
...
size_t znzread(void* buf, size_t size, size_t nmemb, znzFile file)
{
  ...
  if (file->zfptr!=NULL) {
    ...
    while( remain > 0 ) {
       ...
       if( nread < 0 )
	 return (size_t) nread; /* returns -1 on error */
...
int znzprintf(znzFile stream, const char *format, ...)
{
  ...
  if (stream->zfptr!=NULL) {
    ...
    size = (size_t) (strlen(format) + 1000000);  /* overkill I hope */
    tmpstr = (char *) mycalloc(1L, size);
    if( tmpstr == NULL ){
       fprintf(stderr,"** ERROR: znzprintf failed to alloc %lu bytes\n", size);
       return retval;
    }
